   ...
   foreach ($File in $objFolder.items()) 
    {  
     $FileMetaData = New-Object PSOBJECT 
      #for ($a ; $a  -le 266; $a++) 
      for ($a ; $a  -le 10; $a++) # just stopping at the info i need, to spped up things
       {  
...
try {
    $path = $ARGS[0] + "\"
    $filelist = Get-FileMetaData $path

    foreach ($file in $filelist) {
        # skip the file if it already has a proper format
        try {
            [datetime]::ParseExact($file.Name.Substring(0,15), "yyyyMMdd_HHmmss", $null) | Out-Null
        }
        catch {
            # todo: writing your entire code logic in an exception handler is not really the best programming practice

            # get the timestamp
            $pattern = ((Get-Culture).DateTimeFormat).ShortDatePattern + " " + ((Get-Culture).DateTimeFormat).ShortTimePattern
            $timestamp = ([datetime]::ParseExact($file.'Date modified', $pattern, $null)).ToString("u")
            $timestamp = $timestamp.Replace(" ", "_").Replace("-", "").Replace(":", "").Replace("Z", "")

            # local variables to work with
            $filename = [io.path]::GetFileNameWithoutExtension($file.Name)
            $fileext = [io.path]::GetExtension($file.Name)

            # if the file already exist, append a random suffix
            $newfile = $path + $timestamp + $fileext
            if( Test-Path $newfile ) {
                $timestamp = $timestamp + "_" + (Get-Random)
            }

            # if all ok, just rename the file
            $oldfile = $path + $filename + $fileext
            $newfile = $path + $timestamp + $fileext
            if( -Not ((Get-Item $oldfile) -is [System.IO.DirectoryInfo]) ) { # dont rename folders
